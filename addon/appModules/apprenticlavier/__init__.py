...
def getATaperAndDejaTapeControlID():
	...
	if (oDeb == None) or len(oDeb.children) <2:
		return(-1,-1)
	o= oDeb.lastChild
	if o: 
...
class InOtherWindow(Window):
	...
	def event_valueChange(self):
		...
		if self.value != None and len(self.value) or controlID ==5:  
			StopTimer(GB_timer)
...
class InBienvenueWindow(Window):
	...
	def event_gainFocus(self):
		...
		if controlID == 7:
			...
			SayValue("Quitter")
		elif controlID ==  5 and (self.value == None or len(self.value) == 0):
...
class InLessonWindow(Window):
	...
	def GetInfos(self):
		...
		#if sMotCourant == sCaractereCourant:
			#sMotCourant = ""
...
class InLessonByKeyWindow(Window):
	...
	def _caretMovementScriptHelper(self, gesture, unit):
		print ("gesture: %s"%gesture.mainKeyName)
		gesture.send()
	def event_caret (self):
		pass
...
class AppModule(appModuleHandler.AppModule): 
	...
	def chooseNVDAObjectOverlayClasses(self, obj, clsList):
		...
		cls= "inchangee"
		if GB_inGetTopLevelFunction >0: return
	...
	def event_NVDAObject_init(self, obj):
		...
		pass
		#if  obj.windowClassName == "ThunderRTForm" and obj.childCount == 7:
			#if obj.name is None: obj.name = " "
	...
	def event_foreground(self, obj, nextHandler):
		...
		if not GB_moduleHasFocus:
			...
			name = obj.name
			if name != None and (u"leçon" in name[:6].lower()):
	...
	def event_gainFocus(selwf, obj, nextHandler):
		...
		if obj.windowClassName == "ThunderRTForm" and obj.name is None and  obj.childCount in [7, 9]: return
		nextHandler()
